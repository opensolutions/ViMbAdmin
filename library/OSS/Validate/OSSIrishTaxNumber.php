 ...
 * @license    http://www.opensolutions.ie/licenses/new-bsd New BSD License
 */
class OSS_Validate_OSSIrishTaxNumber extends Zend_Validate_Abstract
{
    ...
    protected $_messageTemplates = array(
        self::NOT_VALID => 'This is not a valid Irish tax / PAYE number.'
    );


    /**
     * Validates a given tax / VAT / PAYE number for the correct format. This
    ...
    {
        // 1234567X or 1X34567X are valid (includes one or two letters either the last or second + last)

        if( preg_match( '/^\d{7}[a-z]$/i', $value ) )
        {
        ...
        if( preg_match( '/^\d[a-z]\d{5}[a-z]$/i', $value ) )
        {
            $vRecombined = "0" . substr( $value, 2, 5 ) . substr( $value, 0, 1 ) . substr( $value, 7, 1 );

            if( self::checkMOD23( $vRecombined ) === false )
            {
                $this->_error( self::NOT_VALID );
                return false;
            }
