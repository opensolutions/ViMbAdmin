 ...
 * @subpackage Form
 */
class ViMbAdmin_Form_Admin_AddDomain extends ViMbAdmin_Form
{
    ...
    public function __construct( $options = null, $domainList, $admin )
    {
        ...
        $adminId = $this
                        ->createElement( 'hidden', 'adminId' )
                        ->setValue( $admin->id );

        $domain = $this
                        ->createElement( 'select', 'domain' )
                        ->setOptions( array( 'multiOptions' => $domainList ) ) // array('' => _( '- select -' ) ) + $domainList
                        ->setLabel( _( 'Domain' ) )
